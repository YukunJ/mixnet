...
uint64_t get_curr_time_ms() {
    ...
    return (uint64_t)(tv.tv_sec) * SECOND_TO_MILL + (uint64_t)(tv.tv_usec) / MILL_TO_MICRO;
}

/**
 * Initialize the STP information to believe that self is the root of the topology
 ...
 */
mixnet_packet* generate_stp_packet(
 ...
 */
void broadcast_stp(void *handle, const struct mixnet_node_config *config_ptr, mixnet_address root_address,
        uint16_t path_length, mixnet_address self_address) {
    ...
    for (uint16_t i = 0; i < num_neighbors; i++) {
        mixnet_packet *packet = generate_stp_packet(root_address, path_length, self_address);
...
void broadcast_flood(void *handle, const struct mixnet_node_config *config_ptr,
        bool *is_active_link, uint16_t num_neighbors, uint16_t source_port) {
    for (uint16_t i = 0; i < num_neighbors; i++) {
 ...
 */
bool is_better_root(mixnet_packet_stp *stp_packet,
                    stp_info_t *stp_info) {
 ...
 */
bool is_potential_child(mixnet_packet_stp *stp_packet,
                        stp_info_t *stp_info) {
...
void run_node(void *handle,
    ...
    const uint64_t root_hello_interval_ms = (uint64_t)config.root_hello_interval_ms;
    const uint64_t reelection_interval_ms = (uint64_t)config.reelection_interval_ms;
    ...
    broadcast_stp(handle, &config, stp_info.root, stp_info.root_distance, config.node_addr);
    ...
    while (*keep_running) {
        ...
        if (value != 0) {
            ...
            // STP packet
                ...
                // if we have a better root, update the stp_info, set received_hello_from_root
                // indicating we want to broadcast our potential children about root information
                // update my knowledge about active links
                if (is_better_root(stp_packet, &stp_info)) {
                    ...
                    received_hello_from_root = true;
                    is_active_link[port] = true;
                    // else if the stp packet is from a potential child, we update our knowledge about active links
                ...
                // else I am not the root
                    ...
                    // if I have received root's hello from root, I need to broadcast it to everyone
                    ...
                    // I would assume that I am the root again. I would initialize myself as root
                    // update is_active_link, and broadcast, reset last time sending hello
                    } else if (curr_time - last_time_receiving_hello >= reelection_interval_ms) {
                        init_stp_info(&stp_info, config.node_addr);
                        init_is_active_link(is_active_link, num_neighbors);
                        broadcast_stp(handle, &config, stp_info.root, stp_info.root_distance, config.node_addr);
                        last_time_sending_hello = curr_time;
                ...
                // if the FLOOD package is generated by users, broadcast to all active link
                // if the FlOOD package is from an active link, broadcast to all other active links
                if (port == num_neighbors || (port < num_neighbors && is_active_link[port])) {
