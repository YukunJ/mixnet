 ...
 * It contains a dynamically-expanding vector which supports basic vector
 * operation and a graph_node which is essentially a linked list node containing
 * a vector of neighbors and a graph data structure for storing the neighbors of
 * each node in the graph and a simplified version of Dijkstra's algorithm for
 * source routing
 ...
 * @param equal_comp function pointer to a func if two neighbors are the same,
 * true if the same
 * @return true if the addition is successful, false if already exists
 */
bool graph_node_add_neighbor(graph_node_t *node, ELEMENT_TYPE new_neighbor,
 ...
 * caller should make sure that there is no existing vertex of this host
 ...
 */
dijkstra_t *create_dijkstra_triple(mixnet_address destination,
                                   mixnet_address last_hop, int64_t distance) {
...
/**
 * Function comparator to test against if two mixnet address pointer are the
 * same
 */
 ...
 * @return pointer to a vector containing result triples, user should free it
 * after usage by 'free_vector()'
 */
vector_t *dijkstra_shortest_path(graph_t *graph, mixnet_address source) {
  ...
  vec_push_back(progress, create_dijkstra_triple(
                              source, source, 0));  // source node is 0 distance
  graph_node_t *curr = graph->head->next;
  while (curr) {
    // add a new node if it's unique
    if (vec_find(progress, curr->host, dijkstra_triple_equal) == -1) {
      vec_push_back(progress, create_dijkstra_triple(
                                  *(mixnet_address *)(curr->host),
                                  *(mixnet_address *)(curr->host), INFINITY));
    }
    for (int64_t i = 0; i < graph_node_size(curr); i++) {
      ...
      if (vec_find(progress, neighbor_ptr, dijkstra_triple_equal) == -1) {
        vec_push_back(progress, create_dijkstra_triple(
                                    *neighbor_ptr, *neighbor_ptr, INFINITY));
      }
    }
    curr = curr->next;
  }

  // fetch smallest distance node each time, and try update neighbor-reaching
  ...
  while (vec_size(progress) > 0) {
    ...
    vec_push_back(finished, create_dijkstra_triple(curr_destination,
                                                   curr_last_hop, curr_dist));
    // grab all the neighbors and try update, each hop is 1 distance by
    // assumption
    ...
    for (int64_t i = 0; i < graph_node_size(node); i++) {
      ...
      int64_t neighbor_idx =
          vec_find(progress, neighbor, dijkstra_triple_equal);
      if (neighbor_idx != -1) {
        ...
        if (1 + curr_dist < neighbor_triple->distance) {
          ...
          dijkstra_t *new_triple = create_dijkstra_triple(
              neighbor_triple->destination, curr_destination, 1 + curr_dist);
...
/**
 * Construct the shortest path from source to destination according to
 * Dijkstra's result
 ...
 */
vector_t *construct_path(vector_t *dijkstra_shortest_triple,
                         mixnet_address source, mixnet_address destination) {
  vector_t *path = create_vector();
  mixnet_address to_find = destination;
  while (to_find != source) {
    int64_t idx =
        vec_find(dijkstra_shortest_triple, &to_find, dijkstra_triple_equal);
    assert(idx != -1);
    mixnet_address *curr_hop = (mixnet_address *)malloc(sizeof(mixnet_address));
    *curr_hop =
        ((dijkstra_t *)vec_get(dijkstra_shortest_triple, idx))->destination;
