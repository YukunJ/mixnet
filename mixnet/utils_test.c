...
void int_printer(ELEMENT_TYPE e) { printf("%d ", *(int *)e); }

void graph_insert_helper(graph_t *graph, mixnet_address host,
...
int main(int argc, const char **argv) {
  printf(
      "[A]. Start testing the implementation of dynamically-expanding "
      "vector\n");
  ...
  printf(
      "vector's size and capacity are correct\n============================\n");
  ...
  printf(
      "vector's find func returns correct "
      "index\n============================\n");
  ...
  printf(
      "vector's find_best func returns best "
      "index\n============================\n");
  ...
  printf(
      "vector's remove func works correctly\n============================\n");

  printf(
      "5. vector's clear method should clear out the data and set capacity "
  ...
  printf(
      "[B]. Start testing the implementation of graph node relying on "
      "vector\n");
  ...
  *host_id = 0;
  ...
  printf(
      "1. Add 1 to 5 as host 0's neighbors, should all be successful "
      "addition\n");
  ...
  printf(
      "2. Add again 1 to 5 as host 0's neighbors, since they already exists, "
  ...
  printf(
      "4. graph node's clear method should clear out the neighbor and set back "
      "to default\n");
  graph_node_clear(node);
  assert(graph_node_size(node) == 0 &&
         node->neighbors->capacity == INITIAL_CAPACITY);
  ...
  printf(
      "[C]. Start testing the implementation of graph node relying on "
      "graph_node\n");
  ...
  printf(
      "If the graph looks as you expect, then the test is considered "
      "passed!\n");
  ...
  printf(
      "[D]. Start testing the implementation of Dijkstra's shortest path "
      "algorithm on last graph\n");
  ...
  printf(
      "If the above distance information looks as you expect, then the test is "
      "considered passed!\n");

  printf(
      "2. Given the Dijkstra's triple, we try to reconstruct the whole "
      "shortest path\n");
  ...
  printf(
      "If the above shortest path looks as you expect, then the test is "
      "considered passed!\n");
  free_vector(result);
  free_graph(graph);
