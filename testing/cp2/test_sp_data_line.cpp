...
void pcap(orchestrator* orchestrator,
    ...
    if (packet->type == PACKET_TYPE_DATA) {
        ...
        pcap_check_success &= (header->fragment_id == 7);
        int src_idx = get_index(packet->src_address, mixaddrs);
        pcap_check_success &= (src_idx != -1) && (src_idx < 7);
        pcap_check_success &= (packet->dst_address == mixaddrs[7]);

        pcap_check_success &= (received[src_idx] == 0);
        if (pcap_check_success) { received[src_idx]++; }

        pcap_check_success &= check_route(
            rh, (mixaddrs.data() + src_idx + 1), (6 - src_idx));
...
void testcase(orchestrator* orchestrator) {
    ...
    // Try the first 7 nodes as source
    for (size_t i = 0; i < 7; i++) {
        DIE_ON_ERROR(orchestrator->send_packet(
            i, 7, PACKET_TYPE_DATA, data[i]));
    }
...
int main(int argc, char **argv) {
    ...
    std::cout << "[Test] Starting test_sp_data_line..." << std::endl;
    orchestrator.run();
    std::cout << ((retcode == TEST_ERROR_NONE) ?
        "Nodes returned OK" : "Nodes returned error") << std::endl;

    bool success = ((pcap_count == 7) && pcap_check_success);
